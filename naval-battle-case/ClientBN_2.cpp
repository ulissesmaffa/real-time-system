...
int main() {
    ...
    printf("Cliente 2: Solicitação para ingresso em batalha naval enviada!\n");

    //4:RECV INICIAL
    //Confirmação de ingresso no jogo através do envio do tabuleiro pelo servidor
    recv(s2, &tab_jogadores, sizeof(tab_jogadores), 0);
    ...
    */
    ...
    while(k!=24){
        ...
        if(status==2){
            ...
            scanf("%d %d",&lin,&col);
            send(s2, &lin, sizeof(lin), 0);
            send(s2, &col, sizeof(col), 0);
            printf("Cliente 2: ataque LxC:%dx%d enviado para servidor!\n",lin,col);
        }else{
            printf("aguarde sua vez! o cliente %d esta realizando sua jogada\n",status);
        }

        recv(s2, &tab_jogadores, sizeof(tab_jogadores), 0);//5.3
    ...
    //FIM DA CONEXAO
    printf("\nCliente 2: FORAM 24 TENTATIVAS AO TOTAL, FIM DE JOGO!\n");
    close (s2);
}