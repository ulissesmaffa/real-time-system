...
int main() {
    ...
    printf("Cliente 1: Solicitação para ingresso em batalha naval enviada!\n");

    //4:RECV INICIAL
    //Confirmação de ingresso no jogo através do envio do tabuleiro pelo servidor
    recv(s1, &tab_jogadores, sizeof(tab_jogadores), 0);
    ...
    5.2 => verifica se eh sua vez de jogar 
    5.3 => envia jogada ou campo vazio de aguardando jogada
    5.4 => receb tabuleiro atualizado do servidor
    */
    ...
    while(k!=24){
        ...
        if(status==1){
            ...
            scanf("%d %d",&lin,&col);
            send(s1, &lin, sizeof(lin), 0);
            send(s1, &col, sizeof(col), 0);
            printf("Cliente 1: ataque LxC:%dx%d enviado para servidor!\n",lin,col);
        }else{
            printf("aguarde sua vez! o cliente %d esta realizando sua jogada\n",status);
        }

        recv(s1, &tab_jogadores, sizeof(tab_jogadores), 0);//5.3
    ...
    //FIM DA CONEXAO
    printf("\nCliente 1: FORAM 24 TENTATIVAS AO TOTAL, FIM DE JOGO!\n");
    close (s1);
}